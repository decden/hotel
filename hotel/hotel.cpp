...
namespace hotel
{
  ...
  RoomCategory::RoomCategory(const std::string& shortCode, const std::string& name)
      : _hotel(nullptr), _shortCode(shortCode), _name(name)
  {
  }
  ...
  void Hotel::addRoomCategory(std::unique_ptr<RoomCategory> category)
  {
    ...
    if (existingCategory != _categories.end())
      std::cerr << "Category already registered! Category short code: " << category->shortCode()
                << ", name: " << (*existingCategory)->name() << std::endl;
    else
    {
  ...
  std::vector<std::unique_ptr<RoomCategory>>::iterator Hotel::getCategoryByShortCode(const std::string& shortCode)
  {
    return std::find_if(
        ...
        [shortCode](const std::unique_ptr<RoomCategory>& category) { return category->shortCode() == shortCode; });
  }
