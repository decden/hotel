...
std::vector<std::unique_ptr<Hotel>> SqliteStorage::loadHotels()
{
  ...
  for (auto &hotel : results)
  {
    ...
    {
      int id; std::string short_code; std::string name;
    ...
    {
      int id; int category_id; std::string name;
      ...
      if (category)
        ...
        std::cerr << "Did not find category with id " << category_id << " in hotel " << hotel->name() << " for room " << name << std::endl;
    }
  }
...
std::unique_ptr<PlanningBoard> SqliteStorage::loadPlanning(const std::vector<int>& roomIds)
{
  ...
  {
    int reservationId; std::string description; int atomId; int roomId; boost::gregorian::date dateFrom; boost::gregorian::date dateTo;
    ...
    if (current == nullptr || current->id() != reservationId)
    {
      ...
      current = std::make_unique<hotel::Reservation>(description, roomId, boost::gregorian::date_period(dateFrom, dateTo));
...
void SqliteStorage::prepareQueries()
{
  ...
  _statements.emplace("room_category.insert", SqliteStatement(_db, "INSERT INTO h_room_category (hotel_id, short_code, name) VALUES (?, ?, ?);"));
  _statements.emplace("room_category.by_hotel_id", SqliteStatement(_db, "SELECT id, short_code, name FROM h_room_category WHERE hotel_id = ?;"));
  _statements.emplace("room.insert", SqliteStatement(_db, "INSERT INTO h_room (hotel_id, category_id, name) VALUES (?, ?, ?);"));
  _statements.emplace("room.by_hotel_id", SqliteStatement(_db, "SELECT id, category_id, name FROM h_room WHERE hotel_id = ?;"));

  _statements.emplace("reservation_and_atoms.all", SqliteStatement(_db, "SELECT r.id, r.description, a.id, a.room_id, a.date_from, a.date_to FROM h_reservation as r, h_reservation_atom as a WHERE a.reservation_id = r.id ORDER BY r.id, a.date_from;"));
